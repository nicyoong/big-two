    ...
    def choose_move(self, observation: "Observation") -> Move | PassMove:
        ...
        for cards in legal_plays:
            ...
            score = (
                ...
                + _phase_control_penalty(observation, move, self.personality)
        ...
        if _phase_should_pass(observation, best_move, best_score, self.personality):
            return PassMove()
        return best_move
...
def _opening_adjustment(observation: "Observation", move: Move, personality: Personality | None = None) -> int:
    ...
    # Note: Use per-instance values if provided, otherwise defaults.
    score += p.penalty_two * (OPENING_PENALTY_TWO / 50) * sum(1 for card in move.cards if card.rank == Rank.TWO)
    score += p.penalty_ace * (OPENING_PENALTY_ACE / 20) * sum(1 for card in move.cards if card.rank == Rank.ACE)
