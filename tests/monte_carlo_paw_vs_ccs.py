...
from phase_awareness import PhaseAwareness
from control_card_strategy import ControlCardStrategy
...
def test_phase_awareness_against_three_control_card_strategies_monte_carlo() -> None:
    ...
    print("")
    print("Monte Carlo: PhaseAwareness vs 3 ControlCardStrategy")
    print(f"Games: {GAME_COUNT}")
    print(f"PhaseAwareness wins: {lowest_valid_wins} ({lowest_valid_wins / GAME_COUNT:.1%})")
    print(f"ControlCardStrategy wins: {random_wins} ({random_wins / GAME_COUNT:.1%})")
    print("Wins by seat:")
...
def run_game(game_number: int) -> GameResult:
    ...
    game.seats = [
        PlayerSeat(
            ...
            name="PhaseAwareness",
            kind="logic",
            strategy=PhaseAwareness(),
        ),
        PlayerSeat(
            seat_id="seat-2",
            name="ControlCardStrategy 1",
            kind="logic",
            strategy=ControlCardStrategy(),
        ),
        PlayerSeat(
            seat_id="seat-3",
            name="ControlCardStrategy 2",
            kind="logic",
            strategy=ControlCardStrategy(),
        ),
        PlayerSeat(
            seat_id="seat-4",
            name="ControlCardStrategy 3",
            kind="logic",
            strategy=ControlCardStrategy(),
        ),
    ]
