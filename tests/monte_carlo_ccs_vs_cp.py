...
def test_control_card_strategy_against_three_combo_preservations_monte_carlo() -> None:
    ...
    print("")
    print("Monte Carlo: ControlCardStrategy vs 3 ComboPreservation")
    print(f"Games: {GAME_COUNT}")
    print(f"ControlCardStrategy wins: {lowest_valid_wins} ({lowest_valid_wins / GAME_COUNT:.1%})")
    print(f"ComboPreservation wins: {random_wins} ({random_wins / GAME_COUNT:.1%})")
    print("Wins by seat:")
...
def run_game(game_number: int) -> GameResult:
    ...
    game.seats = [
        PlayerSeat(
            seat_id="seat-1",
            name="ControlCardStrategy",
            kind="logic",
            strategy=ControlCardStrategy(),
        ),
        PlayerSeat(
            seat_id="seat-2",
            name="ComboPreservation 1",
            kind="logic",
            strategy=ComboPreservation(),
        ),
        PlayerSeat(
            seat_id="seat-3",
            name="ComboPreservation 2",
            kind="logic",
            strategy=ComboPreservation(),
        ),
        PlayerSeat(
            seat_id="seat-4",
            name="ComboPreservation 3",
            kind="logic",
            strategy=ComboPreservation(),
        ),
    ]
