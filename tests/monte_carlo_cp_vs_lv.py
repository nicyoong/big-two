...
def test_combo_preserving_bot_against_three_lowest_valid_bots_monte_carlo() -> None:
    ...
    print("")
    print("Monte Carlo: ComboPreservingBot vs 3 LowestValidBot")
    print(f"Games: {GAME_COUNT}")
    print(f"ComboPreservingBot wins: {lowest_valid_wins} ({lowest_valid_wins / GAME_COUNT:.1%})")
    print(f"LowestValidBot wins: {random_wins} ({random_wins / GAME_COUNT:.1%})")
    print("Wins by seat:")
...
def run_game(game_number: int) -> GameResult:
    ...
    game.seats = [
        ...
        PlayerSeat(
            seat_id="seat-2",
            name="LowestValidBot 1",
            kind="bot",
            bot_brain=LowestValidBot(),
        ),
        PlayerSeat(
            seat_id="seat-3",
            name="LowestValidBot 2",
            kind="bot",
            bot_brain=LowestValidBot(),
        ),
        PlayerSeat(
            seat_id="seat-4",
            name="LowestValidBot 3",
            kind="bot",
            bot_brain=LowestValidBot(),
        ),
    ]
