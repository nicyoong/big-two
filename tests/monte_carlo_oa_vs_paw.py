...
def test_opponent_aware_bot_against_three_phase_awareness_monte_carlo() -> None:
    ...
    print("")
    print("Monte Carlo: OpponentAwareBot vs 3 PhaseAwareness")
    print(f"Games: {GAME_COUNT}")
    print(f"OpponentAwareBot wins: {lowest_valid_wins} ({lowest_valid_wins / GAME_COUNT:.1%})")
    print(f"PhaseAwareness wins: {random_wins} ({random_wins / GAME_COUNT:.1%})")
    print("Wins by seat:")
...
def run_game(game_number: int) -> GameResult:
    ...
    game.seats = [
        PlayerSeat(
            seat_id="seat-1",
            name="OpponentAwareBot",
            kind="logic",
            strategy=OpponentAwareBot(),
        ),
        PlayerSeat(
            seat_id="seat-2",
            name="PhaseAwareness 1",
            kind="logic",
            strategy=PhaseAwareness(),
        ),
        PlayerSeat(
            seat_id="seat-3",
            name="PhaseAwareness 2",
            kind="logic",
            strategy=PhaseAwareness(),
        ),
        PlayerSeat(
            seat_id="seat-4",
            name="PhaseAwareness 3",
            kind="logic",
            strategy=PhaseAwareness(),
        ),
    ]
